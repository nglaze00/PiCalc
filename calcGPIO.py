...
#pi.setmode(pi.BCM)
#for x in range(1, 28):
    #if x not in [7,8,9,10,11,18,25]:
        #pi.setup(x, pi.IN, pull_up_down=pi.PUD_UP)
ops = {'+': operator.add,
       '-': operator.sub,          
       '*': operator.mul,                   
       '/': operator.truediv,
...
accept = ['shift','off','left','right','up','down','exit','pi','e','.','(',')','^','spec','exit','/','*','-','+','sqrt','=','dlt','clear','exit','funcs','funcVal','deriv','fr','ans','funcIn','x']
...
def addMult(lst):
    ...
    while y < z: #split parentheses
      ...
      y+=1
    while x < z - 1:
        if ((lst[x+1] in trigs) or (lst[x+1] in {'sqrt', 'ln', '(', 'π', 'e'}) or ('log' in str(lst[x+1]))) and (isfloat(lst[x]) == True or lst[x] in ['π', 'e', ')', '!']):
            print('a')
            print(lst[x])
            print(lst[x+1])
            lst.insert(x+1, '*')
    ...
    if 'x' in lst:
        ...
        z = len(lst)
        while x < z - 1:
            if (lst[x] == 'x') and (lst[x+1] in trigs or lst[x+1] in ['sqrt', 'ln', '(', 'π', 'e', 'x'] or isfloat(lst[x+1]) == True):
                print('b')
                lst.insert(x+1, '*')
                x+=1
            x+=1
...
def repeatTest(lst):
   ...
   for x in range(len(repeaters)-1, -1, -1):
      if repeaters[x] == dec[len(repeaters[x]):2*len(repeaters[x])] == dec[2*len(repeaters[x]):3*len(repeaters[x])]:
   ...
   if doesRepeat == True:
       lst[0] = str(lst[0])
       if repeatDigs not in str(lst[0][repeatLen*(int(100/repeatLen)-5)+2:repeatLen*(int(100/repeatLen)-4)+2]):
           return [lst[0], '', '', '', '']
   
   return [lst[0], final, repeatStart, repeatLen, repeatDigs, doesRepeat] #original, final, start, length of repeating, digits that repeat, repeats?
...
class Funcs:
    ...
    def printSum():
        print(activeS)
        clr()
        add('n = '+ ''.join(sumList[0]), '\n')
        add('____', '\n')
        add('\\', '\n')
        add(' \\ ' + ''.join(funcList[activeS]), '\n')
        add(' /', '\n')
        add('/', '\n')
        add('\u203E\u203E\u203E\u203E', '\n')
        add('x = ' + ''.join(sumList[1]), '\n')
    def printActive():
        clr()
        for y in range(10):
            if funcList[y] != []:
...
def inPi(shift):            ##BUTTON BINDINGS
    while True:
        if pi.input(21) == pi.LOW:
            if shift == False:
                return '0'
            else:
                return 'ans'
        if pi.input(26) == pi.LOW:
            if shift == False:
                return '1'
            else:
                return 'A'
        if pi.input(1) == pi.LOW:
            if shift == False:
                return '2'
            else:
                return 'e'
        if pi.input(3) == pi.LOW:
            if shift == False:
                return '3'
            else:
                return 'pi'
        if pi.input(19) == pi.LOW:
            if shift == False:
                return '4'
            else:
                return 'left'
        if pi.input(24) == pi.LOW:
            if shift == False:
                return '5'
            else:
                return 'up'
        if pi.input(4) == pi.LOW:
            if shift == False:
                return '6'
            else:
                return 'right'
        if pi.input(13) == pi.LOW:
            if shift == False:
                return '7'
            else:
                return 'B'
        if pi.input(23) == pi.LOW:
            if shift == False:
                return '8'
            else:
                return 'down'
        if pi.input(17) == pi.LOW:
            if shift == False:
                return '9'
            else:
                return 'C'
        if pi.input(22) == pi.LOW:
            if shift == False:
                return '='
            else:
                return 'exit'
        if pi.input(15) == pi.LOW:
            if shift == False:
                return '.'
            else:
                pass
        if pi.input(5) == pi.LOW:
            if shift == False:
                return '/'
            else:
                return 'sqrt'
        if pi.input(27) == pi.LOW:
            if shift == False:
                return '*'
            else:
                return '^'
        if pi.input(6) == pi.LOW:
            if shift == False:
                return '+'
            else:
                return '-'
        if pi.input(2) == pi.LOW:
            if shift == False:
                return 'dlt'
            else:
                return 'clear'
        if pi.input(12) == pi.LOW:
            if shift == False:
                return 'x'
            else:
                return 'vars'
        if pi.input(20) == pi.LOW:
            if shift == False:
                return 'on'
            else:
                return 'off'
        if pi.input(16) == pi.LOW:
            if shift == False:
                return 'spec'
            else:
                return 'funcs'
        if pi.input(14) == pi.LOW:
             return 'shift'
        ...
        for num,x in enumerate(funcList):
            for y in x:
                if y == '_':
                    ...
                    return lst, num-1
    if mode == 'sum':
        for num,x in enumerate(sumList):
            for y in x:
                if y == '_':
                    del_(lst)
                    return lst, num-1
...
def dlt(lst): #delete last input character from active list
   for x in range(len(lst)):
       if lst[x] == '_' and x > 0:
           ...
           break
def re(lst, mode):
    os.system('clear')##Change to 'clear' for linux, 'cls' windows
    if mode == 'func':
        clr()
        Funcs.printAll()
    if mode == 'sum':
    ...
    #try: ERROR
        ...
        if charList not in [[], ['error']]:
            ...
            if len(charList) == 1:
                ...
                oList = repeatTest(charList) # [original, repeating format, start, length of repeating, digits that repeat]              UNCOMMENT WHEN FIXED
